...
def get_inputs(imgdir, annodir,  order_log, turn_i):
    inputs = []
    img_test_path = os.path.join(imgdir, "Image")
    anno_test_path = os.path.join(annodir, "Anno")
    msk_test_path = os.path.join(annodir, "mask")

    with open(order_log, 'r') as f:
        img_turn = json.load(f)

    for file in tqdm(os.listdir(anno_test_path)):
        img_name = file.split('.')[0]
        img_path = os.path.join(img_test_path, f'{img_name}.jpg')
        
        anno_file = os.path.join(anno_test_path, file)
        ...
        for idex_order, order in enumerate(img_turn[img_name]):
            ...
            exam_bbox = anno["exam_bbox"][idex_order]
            exam_mask_path = os.path.join(msk_test_path, img_name, f"mask{idex_order}.png")
...
if __name__ == "__main__":
    ...
    inputs = get_inputs(opt.imgdir, opt.annodir,  opt.order_log, opt.turn)
