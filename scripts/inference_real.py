...
def get_inputs(testdir, locationdir):
    ...
    anno_bbox_test_path = os.path.join(locationdir, "custom_output_k3")
    exam_bbox_folder_path = os.path.join(locationdir, "boxes_sam")
    mask_bbox_folder_path = os.path.join(locationdir, "masks_sam")
    ...
    for img, img_info in tqdm(info.items()):
        ...
        exam_mask_path = os.path.join(mask_bbox_folder_path, img.split('.')[0], f"mask{mask_id}.png")
        ...
        #get location bbox
        loc_bbox_path = os.path.join(anno_bbox_test_path, img.replace('.jpg', '.json'))
