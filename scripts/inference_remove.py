...
from ldm.data.open_images import get_tensor, get_tensor_clip, get_bbox_tensor, bbox2mask, mask2bbox, constant_pad_bbox
...
def generate_image_batch(bg_path, bbox):
    bg_img     = Image.open(bg_path).convert('RGB')
    bg_w, bg_h = bg_img.size
    bg_t       = sd_transform(bg_img)

    # fg_img = np.zeros_like(np.array(bg_img))
    # fg_img = Image.fromarray(fg_img)
    fg_img = Image.new("RGB", (bg_w, bg_h), (0, 0, 0))
    
    fg_t       = clip_transform(fg_img)
    bbox = constant_pad_bbox(bbox, bg_w, bg_h, value=5)
...
def draw_bbox_on_background(image_nps, norm_bboxes, color=(255,215,0), thickness=3):
    dst_list = []
    if norm_bboxes.ndim == 2:
        norm_bboxes = norm_bboxes[:, None, :]

    B = image_nps.shape[0]
    K = norm_bboxes.shape[1]
    ...
    for i in range(B):
        ...
        for j in range(K):
            x1 = int(norm_bboxes[i, j, 0] * w)
            y1 = int(norm_bboxes[i, j, 1] * h)
            x2 = int(norm_bboxes[i, j, 2] * w)
            y2 = int(norm_bboxes[i, j, 3] * h)
...
def argument_parse():
    ...
    parser.add_argument(
        "--ckpt_objstitch",
        default="./checkpoints/objectstitch.ckpt",
        help="path to checkpoint of objectstitch model",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def parse_input_bbox(opt, img_name):
    mask_txt = os.path.join(opt.testdir, 'box/test', img_name.split('.')[0] + '.txt')
    if os.path.exists(mask_txt):
...
def get_test_list(opt, mode="all", numcase=None):
    if mode == "all":
        test_list  = os.listdir(os.path.join(opt.testdir, 'inpaint/test'))
    # else:
    #     test_list = [f for f in os.listdir(os.path.join(opt.testdir, 'inpaint/test')) if f.endswith(mode)]
    else:
        test_list = [f for f in os.listdir(os.path.join(opt.testdir, 'inpaint/test')) if f.startswith(mode)]
    
    if numcase is not None and numcase < len(test_list):
        # Fix seed để reproducible
        seed = opt.seed if hasattr(opt, "seed") else 42
        random.seed(seed)
        num_case = numcase if numcase else len(test_list)
        # Random chọn num_case phần tử (không lặp)
        num_case = min(num_case, len(test_list))
        test_list = random.sample(test_list, num_case)

    return test_list

if __name__ == "__main__":
    opt = argument_parse()
    weight_path = opt.ckpt_objstitch
    ...
    test_list = get_test_list(opt, mode="2_")
    print('find {} pairs of test samples'.format(len(test_list)))
    os.makedirs(opt.outdir, exist_ok=True)
    
    for img_name in test_list:
        bg_path    = os.path.join(opt.testdir, 'inpaint/test', img_name)
      ...
      #  fgmask_path= None if not os.path.exists(fgmask_path) else fgmask_path
        start = time.time()
        batch = generate_image_batch(bg_path, bbox)
        ...
        if not opt.skip_grid:
            ...
            grid_path = os.path.join(opt.outdir, img_name.split('.')[0] + '_grid.jpg')
