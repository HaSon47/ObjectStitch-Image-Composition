...
def generate_image_batch(bg_path, bbox):
    ...
    bbox = constant_pad_bbox(bbox, bg_w, bg_h, value=75)
...
def get_inputs(testdir, locationdir):
    inputs = []
    img_test_path = os.path.join(testdir, "input")

    anno_class_test_path = os.path.join(testdir, "annotations.json")
    exam_bbox_folder_path = os.path.join(locationdir, "boxes_sam")

    with open(anno_class_test_path, 'r') as f:
        info = json.load(f)
    for img, img_info in tqdm(info.items()):
        img_path = os.path.join(img_test_path, img)
        # get example path and bbox
        mask_id = img_info["mask_id"]
        exam_bbox_path = os.path.join(exam_bbox_folder_path, img.replace('.jpg', '.json'))
        with open(exam_bbox_path, 'r') as f:
            anno_exam = json.load(f)
        exam_bbox = anno_exam["boxes"][mask_id]
...
if __name__ == "__main__":
    ...
    inputs = get_inputs(opt.testdir, opt.locationdir)
