...
if __name__ == "__main__":
    ...
    img_out_path = os.path.join(opt.outdir, 'img')
    msk_out_path = os.path.join(opt.outdir, 'mask')
    grid_out_path = os.path.join(opt.outdir, 'grid')
    os.makedirs(img_out_path, exist_ok=True)
    os.makedirs(msk_out_path, exist_ok=True)
    ...
    for img_name in tqdm(test_list):
        ...
        res_img_path = os.path.join(img_out_path, img_name)
        save_image_out(comp_img[-1], res_img_path)

        mask = batch["bg_mask"].squeeze(0)
        res_msk_path = os.path.join(msk_out_path, img_name)
        ...
        if not opt.skip_grid:
            ...
            grid_path = os.path.join(grid_out_path, img_name.split('.')[0] + f'.jpg')
