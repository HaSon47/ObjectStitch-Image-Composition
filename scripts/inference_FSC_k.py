'''
Thử bài toán chèn k vật thể cho ObjectStitch
Cách tiếp cận: ảnh gt mask k bbox
'''
...
from ldm.data.open_images import get_tensor, get_tensor_clip
...
def get_bbox_tensor(bboxes, width, height):
    """
    Args:
    ...
    Returns:
        tensor shape (N, 4), normalized xyxy
    """
    if len(bboxes) == 0:
        return torch.zeros((0, 4), dtype=torch.float32)

    norm_bbox = torch.tensor(bboxes, dtype=torch.float32)

    norm_bbox[:, 0::2] /= width   # x coords
    norm_bbox[:, 1::2] /= height  # y coords
...
def generate_image_batch(img_folder_path, k):
    ...
    mask       = Image.fromarray(bboxes2mask(bboxes, bg_w, bg_h))
    mask_t     = mask_transform(mask)
    mask_t     = torch.where(mask_t > 0.5, 1, 0).float()
    inpaint_t  = bg_t * (1 - mask_t)
    bbox_t     = get_bbox_tensor(bboxes, bg_w, bg_h)
